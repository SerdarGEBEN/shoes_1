...
const Size = ({ selected, setSelected }: FilterProps) => {
  ...
  useEffect(() => {
    if (selected.length > 0) {
      // seçili elemanları url'e ekle ve aralarına , koy.
      params.set("size", selected.join(","));
    } else {
      // seçili eleman yoksa parameteryi kaldır.
      params.delete("size");
    }
  ...
  // üzerine tıklanılan seçeneği state'de yoksa ekle varsa kaldır.
